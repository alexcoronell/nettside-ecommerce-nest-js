...
describe('TagController (e2e) [PATCH]', () => {
  ...
  describe('PATCH Tag', () => {
    it('/:id should update a tag with admin user', async () => {
      const newTags = generateNewTags(10);
      const dataNewTags = await repo.save(newTags);
    ...
    it('/:id should return Conflict if tag name is already taken', async () => {
      const newTags = await repo.save(generateNewTags(10));

      const tag = newTags[0];
      const id = newTags[1].id;

      const updatedData: UpdateTagDto = {
        name: tag.name,
      };
      try {
        await request(app.getHttpServer()).post(`/tag/${id}`).send(updatedData);
      } catch (error) {
        expect(error).toBeInstanceOf(ConflictException);
        expect(error.message).toBe(
          `The tag NAME ${updatedData.name} is already in use`,
        );
      }
    });
