...
import { createPurchaseDetail } from '@faker/purchaseDetail.faker';
...
describe('PurchaseDetailController (e2e) [POST]', () => {
  ...
  describe('POST PurchaseDetail', () => {
    it('/ should create purchase details, return 201 with admin user', async () => {
      const dto: CreatePurchaseDetailDto[] = products.map((product, index) => ({
    ...
    it('/ should return 401 with seller user', async () => {
      const dto: CreatePurchaseDetailDto[] = products.map((product) => ({
        ...createPurchaseDetail(),
        product: product.id,
        purchase: purchase.id,
      }));
    ...
    it('/ should not create purchase details, return 401 with invalid api key', async () => {
      const dto: CreatePurchaseDetailDto[] = products.map((product) => ({
        ...createPurchaseDetail(),
        product: product.id,
        purchase: purchase.id,
    ...
    it('/ should not create purchase details, return 401 if api key is missing', async () => {
      const dto: CreatePurchaseDetailDto[] = products.map((product) => ({
        ...createPurchaseDetail(),
        product: product.id,
        purchase: purchase.id,
