...
describe('PurchaseDetailController (e2e) [DELETE]', () => {
  ...
  describe('DELETE PurchaseDetail', () => {
    it('/:id should delete a purchase detail with admin user', async () => {
    ...
    it('/:id should return 404 when trying to delete already deleted purchase detail', async () => {
      await repo.update(ID, { isDeleted: true, deletedAt: new Date() });
      const res = await request(app.getHttpServer())
        .delete(`${PATH}/${ID}`)
        .set('x-api-key', API_KEY)
        .set('Authorization', `Bearer ${adminAccessToken}`);
      const { statusCode, message, error } = res.body;
      expect(statusCode).toBe(HTTP_STATUS.NOT_FOUND);
      expect(message).toBe(`The Purchase Detail with ID ${ID} not found`);
      expect(error).toBe(ERRORS.NOT_FOUND);
    });
  });
