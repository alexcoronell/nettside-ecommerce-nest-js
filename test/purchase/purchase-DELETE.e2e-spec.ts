...
describe('PurchaseController (e2e) [DELETE]', () => {
  ...
  describe('DELETE Purchase', () => {
    it('/:id should delete a purchase with admin user', async () => {
    ...
    it('/:id should return 404 when trying to delete already deleted purchase', async () => {
      ...
      const { statusCode, message, error } = res.body;
      expect(statusCode).toBe(HTTP_STATUS.NOT_FOUND);
      expect(message).toBe(`The Purchase with ID ${ID} not found`);
      expect(error).toBe(ERRORS.NOT_FOUND);
    });
  });
