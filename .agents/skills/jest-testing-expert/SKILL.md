...
module.exports = {
  collectCoverageFrom: ['src/**/*.{js,ts,jsx,tsx}', '!src/**/*.d.ts', '!src/**/*.stories.*', '!src/**/index.{js,ts}'],
};
```
...
module.exports = {
  ...
  // Test patterns
  testMatch: ['<rootDir>/src/**/__tests__/**/*.(ts|js)?(x)', '<rootDir>/src/**/?(*.)(test|spec).(ts|js)?(x)'],

  // Coverage configuration
  collectCoverageFrom: ['src/**/*.{ts,tsx}', '!src/**/*.d.ts', '!src/index.tsx', '!src/**/*.stories.{ts,tsx}', '!src/**/__tests__/**', '!src/**/__mocks__/**'],
  coverageThreshold: {
    global: {
      branches: 80,
      functions: 80,
...
test('concurrent operations', async () => {
  const promises = [createUser({ name: 'User1' }), createUser({ name: 'User2' }), createUser({ name: 'User3' })];
...
expect.extend({
  ...
  toHaveBeenCalledWithObjectMatching(received, expected) {
    ...
    const pass = calls.some((call) => call.some((arg) => typeof arg === 'object' && Object.keys(expected).every((key) => arg[key] === expected[key])));
